...
export function GlobalSecrets({ secrets, onAdd, onRemove }: {
  ...
  return (
    <div className="card" style={{ marginTop: 'var(--space-6)', padding: 'var(--space-4)' }}>
      <h3 className="section-title" style={{ fontSize: 15, marginBottom: 'var(--space-1)' }}>Global secrets</h3>
      <p className="section-desc" style={{ marginTop: 0 }}>Reusable API keys stored in your OS keychain (via sbx -g). Applied when a sandbox is created.</p>
      ...
      {secrets.length === 0
        ? <p className="section-desc" style={{ margin: 0, fontSize: 12 }}>No global secrets yet.</p>
        : secrets.map((s) => (
          <div key={s.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 10px', border: '1px solid var(--border)', borderRadius: 'var(--radius-sm)', marginBottom: 4 }}>
            ...
            <button className="btn btn-ghost btn-sm" aria-label="Remove" onClick={() => onRemove(s.id)}>Remove</button>
          </div>
        ))}
    </div>
  )

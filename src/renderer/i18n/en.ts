...
export const en = {
  ...
  credentials: {
    ...
    host: 'Host / Domain',
    hostInvalid: '“{host}” is not a usable host. Enter a bare host, IP, or wildcard — for example api.example.com or *.example.com (no https://, port, or path).',
    envVar: 'Environment Variable',
    envVarTaken: 'That environment variable is already used by another custom secret in this definition. Each one needs its own name. To reach several domains with ONE secret, put them all in that entry’s Host / Domain field, separated by commas.',
    wildcardHint: 'Separate several domains with commas to reach them all with one secret (accounts.google.com, oauth2.googleapis.com). Supports wildcards (*.example.com, **.example.com). The proxy substitutes your value wherever the environment variable appears in requests to those hosts — the agent never sees the real secret.',

...
1. **Custom credentials use `sbx secret set-custom`, NOT kit `serviceAuth`.** The proxy swaps a generated placeholder (`sbx-cs-…`) for the real value wherever it appears in an outbound request to a matching `--host`. The env var is set to the placeholder at sandbox **creation**, so the secret is registered **sandbox-scoped against the app-computed unique name, just before `sbx create`** (see "Env-var delivery" below — no global scope).

   **Env-var delivery — SANDBOX-SCOPED, pre-registered, verified (2026-07-19). Do NOT use global (`-g`).** The app already computes the unique sandbox name in `launchDefinition` *before* opening the terminal, so it registers the secret **scoped to that exact name, before `sbx create`**, and `sbx` auto-populates the in-VM env var at creation — confirmed end-to-end: `sbx secret set-custom spike-test --host postman-echo.com --env DELTA_KEY --value …` (sandbox `spike-test` not yet created) → then `sbx create … --name spike-test` → inside the VM `printenv DELTA_KEY` = `sbx-cs-…` and `curl -H "X-Test: $DELTA_KEY" postman-echo.com` echoed back the **real** value. `sbx secret ls` showed it scoped to `spike-test` only — **no global secret**. Do **not** hand-edit `/etc/sandbox-persistent.sh` — custom-secret env vars aren't injected through that file, and it's sbx-managed/experimental.
   - **Value input:** `sbx secret set-custom` has **no stdin flag** — only `--value <secret>` (marked "visible in shell history"). Spawn it from the main process with an **args array (no shell)**: no shell-history leak; brief `ps` argv exposure of the user's own secret on their own machine — acceptable + documented for a single-user local app. (Built-in `sbx secret set <name> <service>` *does* take stdin → use stdin there, zero exposure.)
   - **Cleanup on remove — REQUIRED:** sandbox-scoped custom secrets are **NOT** deleted when the sandbox is removed (verified: `DELTA_KEY` survived `sbx rm spike-test`). So `instance:remove` (and any relaunch reusing a name) MUST also run `sbx secret rm <name> --host <host> -f` for each custom credential, or scoped secrets accumulate. Add this to the remove flow.
   - **Setting a scoped secret on an already-RUNNING sandbox does NOT update its env var** (sbx warns to update manually) → only the pre-create registration path is viable; never register after create.
   - **Service creds:** use the same per-sandbox scoping — `sbx secret set <name> <service>` (stdin) pre-registered against the computed name. Pattern is identical to the verified set-custom case; confirm service-scoped pre-create during Task 11 implementation.
...
**Mechanism smoke — PASSED (2026-07-19).** Ran `launchDefinition`'s exact `sbx` command sequence against real sbx: secrets registered sandbox-scoped pre-create (service `set` + custom `set-custom`); `sbx create --kit` with the allowlist kit; in-VM custom env var = `sbx-cs-…` sentinel; **injection confirmed** (`x-api-key: smoke-secret-XYZ` echoed back after sending the placeholder); non-allowlisted domain denied (HTTP 000); `instance:remove`'s scoped-secret cleanup removed the custom secret. The GUI/IPC glue on top is covered by the 193 unit tests. Optional remaining: click-through in `npm run dev` for full UI confidence.
